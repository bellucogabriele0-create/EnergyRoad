...
public interface ProductAndServiceRepository extends JpaRepository<ProductAndService, UUID> {
    ...
    List<ProductAndService> findByPrice(BigDecimal price);

    Optional<ProductAndService> findByName(String name);

    List<ProductAndService> findByPriceLessThanEqual(BigDecimal maxPrice);

    List<ProductAndService> findByPriceGreaterThanEqual(BigDecimal minPrice);
}
